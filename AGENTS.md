...
**One transport call per button, and that is a budget constraint rather than a style choice.** A single `evalInObsidian` closure is one CDP `Runtime.evaluate`, capped at ~30 s. The walk used to open a note and click all of its buttons inside ONE closure, so it declared `SETTLE + RESULT × buttonCount` — 34 s for a two-button note, 70 s for a five-button one. Sizing cannot fix that: the multiplier is a runtime DOM fact, so any per-button ceiling small enough for the longest note is too small to be worth having. `openNoteAndListButtonCaptions` therefore spends the settle budget once to enumerate; `clickButtonByCaption` spends **both** budgets once per call — it re-finds its button before clicking it — and the per-note result is accumulated in Node, where nothing is capped. It is that SUM the transport bounds, which is why the file calls `obsidian-dev-utils`' exported `assertClickBudgetsFitTransportCap` at module scope: a pair at or over the cap dies as a bare `script timeout` naming neither budget, and the lint rule cannot see it because the budgets reach the closure through `input`.

**Buttons are addressed by CAPTION, and the enumeration scroll-walks the preview.** Reading view mounts lazily and evicts the sections it scrolls past, so no single scroll position holds a whole note's buttons — `advanceScroll()` steps the preview down by 80% of a viewport on every poll iteration, wraps at the bottom, and the captions seen along the way are ACCUMULATED into a `Set`. Reading ONCE is what this file did until 2026-09-20, and it is why 13 of the vault's 77 buttons had never been clicked by anything: `01 Code buttons.md` yielded 6 of its 11, `43 Code button context.md` 6 of its 9, `37 Invocable scripts.md` 0 of its 2. Both `EXPECTED_NON_OK` entries matched nothing, because the two by-design failures they allowlist sit below the fold — so a green tick meant "no button in the top of any note errors". The run now clicks **77 buttons across 39 notes**, exactly as many per note as its source declares. **Caption addressing is what makes the scroll walk possible**: the previous design parked the enumerated elements on the renderer's `window` and clicked them by index, which cannot survive a walk that unmounts them, and re-finding by caption also survives a `removeAfterExecution` button rewriting and re-rendering its note. Deduplication by caption removes the old double-click on short notes for free.

**The expected count comes from the note's SOURCE, and counting it needs a real fence scanner.** The suite asserts `captions.length >= buttonCount` so that a fence which silently stayed a plain code block fails instead of passing vacuously. Deriving that count with a bare `/^\s*```code-button/gm` — which is what `obsidian-dev-utils`' shared `listNotesWithButtons` does — is wrong **here specifically, because this vault documents code buttons with code buttons**: a ` ```code-button ` nested inside a longer ````markdown fence is a sample being shown to the reader, and `42 Code button config.md` is 15 fences of which **2** are real. An `isRaw` fence renders no button element at all (`code-button-block.ts` leaves `buttonEl` as `null`), so it cannot be enumerated either. `countRenderedButtons` therefore tracks the open fence's backtick-run length and skips both kinds. Counting them would make the assertion unsatisfiable, which gates exactly as much as not asserting at all — and it would also break caption addressing, since with the samples counted in that note shows `(no caption)` ×7 and `Click me` ×4. Correctly counted, every caption in the vault is unique within its note.

**Its walk must recurse, and it excludes notes with no buttons by content rather than by name.** The notes live in numbered group folders, so a top-level-only `readdirSync(...).filter((entry) => entry.isFile())` would find the three notes left at the vault root, click those, and pass — the coverage loss is invisible, because an empty note list makes every assertion vacuous. `collectNotes` therefore recurses and skips `_`- and `.`-prefixed folders plus `08 Working with other plugins/` (whose notes each install a third-party plugin first). It no longer carries a `00 Start.md` / `README.md` name list: a note whose source declares no button is dropped instead, which cannot go stale, means a README that one day gains a button is clicked rather than skipped, and reclaims the render budget those five notes each used to spend timing out. If a change to this suite drops the note count, that is the defect — check the count, not just the green tick.

**Why this file is not `registerDemoVaultButtonSuite`.** It is the specialized copy of the shared suite, and four things keep it one: the fence scanner above, `EXPECTED_NON_OK` (by-design non-`ok` buttons — an error demo, and a `shouldShowSystemMessages: false` button that prints no banner to classify), the `dismissModals` cleanup that lets a button which `await alert(...)` resolve, and excluded FOLDERS rather than notes. `assertClickBudgetsFitTransportCap` and `formatFailures` ARE imported from it, so the two cannot disagree about the cap. Closing those four gaps in the library is filed against `obsidian-dev-utils`; once they land, this file can be deleted in favour of the shared suite.
...
**Pending — delete this paragraph once it has shipped.** One subject in the range since `13.7.1` needs that treatment: `ac50667`, which names a group of repositories by a collective noun that a reader of this one has no list to resolve. Rewrite its entry as `docs: align the debug section's wording with the sibling plugins`. Every other subject in the range was read when this was written, on 2026-09-20, and needed nothing — but the range keeps growing, so that is a statement about a date rather than about the section you are reviewing.
