...
**One transport call per button, and that is a budget constraint rather than a style choice.** A single `evalInObsidian` closure is one CDP `Runtime.evaluate`, capped at ~30 s. The walk used to open a note and click all of its buttons inside ONE closure, so it declared `RENDER_TIMEOUT_MS + BUTTON_TIMEOUT_MS × buttonCount` — 34 s for a two-button note, 70 s for a five-button one. Sizing cannot fix that: the multiplier is a runtime DOM fact, so any per-button ceiling small enough for the longest note is too small to be worth having. `openNoteAndStashButtons` therefore spends the render budget once and parks the enumerated buttons on the renderer's `window`; `clickStashedButton` spends the button budget once per call, addressing them by index; the per-note result is accumulated in Node, where nothing is capped. **The stash is what keeps that faithful** — a DOM element cannot cross the transport, and re-querying per click would address a different element list after a `removeAfterExecution` button rewrote and re-rendered its note. CodeScript Toolkit writes a button's result into its own block element even once detached, which is why holding the original reference works.

**Known gap: the walk enumerates ONE snapshot per note, so buttons below the fold are never clicked.** Reading view mounts lazily and keeps only the sections near the viewport, so the single `runButtons()` query after the render wait sees a note's leading buttons and nothing else — `42 Code button config.md` declares 15 fences and yields 1, `01 Code buttons.md` declares 13 and yields 6; 92 fences across the walked notes, 64 distinct captions ever clicked. The tell is that both `EXPECTED_NON_OK` entries match nothing: the two by-design failures they allowlist sit below the fold and have never run. So a green tick here means "no button in the top of any note errors", not "no button errors". `obsidian-dev-utils`' `script-utils/demo-vault-buttons.ts` — the shared button suite the other plugins run, of which this file is the specialized copy — carries the treatment: scroll-walk the preview on every poll, accumulate the captions seen, and address each button by caption rather than by element. Porting it is expected to surface real red, since those buttons have never been executed by anything.

**Its walk must recurse, and must exclude by basename.** The notes live in numbered group folders, so a top-level-only `readdirSync(...).filter((entry) => entry.isFile())` would find the three notes left at the vault root, click those, and pass — the coverage loss is invisible, because an empty note list makes every assertion vacuous. `collectNotes` therefore recurses, skips `_`- and `.`-prefixed folders plus `08 Working with other plugins/` (whose notes each install a third-party plugin first), and excludes `00 Start.md` / `README.md` at **every** depth rather than only at the top. If a change to this suite drops the note count, that is the defect — check the count, not just the green tick.
