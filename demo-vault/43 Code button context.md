...
While a [code button](<./38 Code buttons.md>) runs, a variable called `codeButtonContext` is in scope. It is the button's handle on itself and on Obsidian: where to draw, how to render markdown, how to edit the note the button lives in, and how to load a temporary plugin. This is what turns a code button from "runs a script" into "builds a small piece of UI inside a note".
...
The same functions are available to plain scripts through the [`codescript-toolkit` module](<./06 codescript-toolkit.md>).
