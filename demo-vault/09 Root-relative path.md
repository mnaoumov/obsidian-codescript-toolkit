...
Which folder `/` means is the **Modules root** setting in the plugin's settings tab. In this vault it is `_assets/CodeScriptToolkit`, so `/rootRelativePath.js` is `_assets/CodeScriptToolkit/rootRelativePath.js`. To address the vault root instead, see [Vault-root-relative path](<./11 Vault-root-relative path.md>).
