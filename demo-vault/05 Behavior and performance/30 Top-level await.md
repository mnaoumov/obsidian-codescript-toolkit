...
A module that awaits at the top level cannot finish loading synchronously, so `require()` cannot load it on any platform — use [`requireAsync()`](<../02 Core functions.md#requireasync>), or pre-load it with [`requireAsyncWrapper()`](<./35 requireAsyncWrapper.md>) if the surrounding code has to stay synchronous.
