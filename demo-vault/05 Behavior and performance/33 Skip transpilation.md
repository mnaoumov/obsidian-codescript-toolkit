...
Everything else is transpiled — `.mjs`, `.ts`/`.cts`/`.mts`, ES modules, files under a `"type": "module"` package, [URLs](<../04 Loading from somewhere else/27 URLs.md>), and any `.js` with no `package.json` nearby. Detection deliberately errs toward transpiling: a wrongly-transpiled module is merely slower, whereas a wrongly-skipped ES module would break.
...
- [ES module](<../02 What you can load/11 ECMAScript modules.md>) `import`/`export` syntax;
- TypeScript syntax;
- [Top-level await](<./30 Top-level await.md>);
- [Dynamic `import()`](<./34 Dynamic import.md>) rewriting to [`requireAsync()`](<../02 Core functions.md#requireasync>).
