...
| Value  | Loads as                                                       |
| ------ | -------------------------------------------------------------- |
| `json` | [JSON files](<./13 JSON files.md>)                             |
| `jsTs` | JavaScript/TypeScript: `.js`/`.cjs`/`.mjs`/`.ts`/`.cts`/`.mts` |
| `md`   | [Markdown files](<./14 Markdown files.md>)                     |
| `node` | [Node binaries](<./15 Node binaries.md>)                       |
| `wasm` | [WebAssembly](<./16 WebAssembly.md>)                           |

The same option is useful for [URLs](<../04 Loading from somewhere else/27 URLs.md>), where the server's `Content-Type` header is often missing or too generic to trust.
