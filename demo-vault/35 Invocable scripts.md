...
Turn a script into a real Obsidian command. Export an `invoke()` function and the script appears in the `Command Palette`, ready to be run, bound to a [hotkey](<./37 Hotkeys.md>), or called from anywhere commands can be called — no plugin to write, no manifest, no reload.
