...
function transformProgram(path: NodePath<Program>): void {
  ...
  if (urlMetas.length > 0 || resolveMetas.length > 0) {
    ...
    const requireFactoryIdentifierName = generateUniqueIdentifier({ path, preferredName: CREATE_REQUIRE_IMPORT, reservedIdentifiers });

    path.node.body.unshift(template.statement.ast(`import ${urlId} from 'url';`));
    if (resolveMetas.length > 0) {
      path.node.body.unshift(template.statement.ast(`import { createRequire as ${requireFactoryIdentifierName} } from 'module';`));
    }
